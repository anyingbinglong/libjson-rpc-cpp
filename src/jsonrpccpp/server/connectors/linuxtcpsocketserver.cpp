...
void LinuxTcpSocketServer::HandleConnection(int connection)
{
    ...
    // 从connection（服务端socket） -- 服务端程序与客户端程序建立的TCP连接中，    
    // 读取：客户端程序向服务端程序，发送的RPC请求消息（对象）
    // 并将读取到的内容，存放在request中保存
    ...
    StreamWriter writer;
    // 通过connection（服务端socket） -- 服务端程序与客户端程序建立的TCP连接，    
    // 传送：服务端给客户端返回的，json格式的响应对象response
